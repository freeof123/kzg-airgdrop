...
from py_ecc.optimized_bls12_381.optimized_curve import G1, Z1, multiply, add, G2, Z2
...
def verify_kzg_proof_impl(
    ...
    X_minus_z = add(
        ...
        multiply(bls.G2(), (BLS_MODULUS - z) % BLS_MODULUS),
    )
    P_minus_y = add(
        bls.bytes48_to_G1(commitment),
        multiply(bls.G1(), (BLS_MODULUS - y) % BLS_MODULUS),
    )
...
#     print("Starting bit reversal permutation...")
#     roots_of_unity_brp = bit_reversal_permutation(ROOTS_OF_UNITY)
#     print(f"Bit reversal permutation took {time.time() - start_time} seconds.")
...
#         # Log how long each loop iteration takes
#         print(f"Iteration {i} took {time.time() - loop_start_time} seconds.")
